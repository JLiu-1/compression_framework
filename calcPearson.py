...
if __name__ == "__main__":
	...
	if is_global:
		...
		pearson = calc_pearson(ori_data, dec_data)

		print("Global Stats:")
		print("Error bound: %.20g" % np.max(np.abs(ori_data - dec_data)))
		print("Pearson: %.10g" % pearson)
		...
		pearsons = np.zeros(num_blocks, dtype = np.double)
		idx = 0
		for ori_block, dec_block in iter_blocks(ori_data, dec_data, block_size, shift):
			cur_pearson = calc_pearson(ori_block, dec_block)
			pearsons[idx] = cur_pearson
			idx += 1
		...
		print("Blockwise Pearson: max value: %.10g, min value: %.10g, mean value (overall Pearson): %.10g, variance: %.10g, standard derivation: %.10g" % calc_stats_single(pearsons) )

		if write_to_file:
			pearsons.tofile(output_path)
			print("%d block Pearsons written to file." % num_blocks)
